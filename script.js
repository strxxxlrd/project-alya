// ================================================
// TRANSLATIONS - ПОЛНАЯ ВЕРСИЯ С ИСПРАВЛЕНИЯМИ
// ================================================
const translations = {
    uk: {
        'project-01-p':'Маркетингова агенція',
        'project-02-p':'Крипто-школа',
        'project-03-p':'Спортивний клуб',
        'project-04-p':'Студія дизайну інтер’єрy',
        
        

        
        
        // Header
        'design-studio': 'Design studio',
        
        // Hero
        'hero-text': 'Лендінги та корпоративні сайти<br />для компаній, що хочуть виглядати<br />сучасно та залучати нових клієнтів',
        'hero-text-mobile': 'Лендінги та корпоративні сайти для компаній, що хочуть виглядати сучасно та залучати нових клієнтів',
        'hero-title': 'Структура.<br /> Дизайн. Результат.',
        'hero-title-mobile': 'Структура. Дизайн. Результат.',
        'discuss-project': 'Обговорити проект',
        
        // About
        'about-us': '[ Про нас ]',
        'about-title': 'Ми допомагаємо компаніям<br />виглядати професійно в онлайнi',
        'about-title-mobile': 'Ми допомагаємо компаніям виглядати професійно в онлайнi',
        'about-desc': 'Поєднуємо стратегію, дизайн та розробку,<br />щоб створювати сайти, які працюють на бізнес',
        'about-desc-mobile': 'Поєднуємо стратегію, дизайн та розробку, щоб створювати сайти, які працюють на бізнес',
        
        // Services
        'service-01-title': 'Односторінковий сайт',
        'service-01-desc': 'Підходить для запуску нового продукту, рекламної кампанії або презентації конкретної послуги. Такий сайт допомагає швидко пояснити пропозицію, показати переваги та привести клієнта до заявки',
        'service-02-title': 'Корпоративний сайт',
        'service-02-desc': 'Підходить для компаній, яким важливо повноцінно представити бізнес в інтернеті. На такому сайті можна розповісти про компанію, послуги, проєкти та створити професійний онлайн-імідж',
        'service-03-title': 'Редизайн сайту',
        'service-03-desc': 'Підходить, якщо сайт вже існує, але виглядає застаріло або незручний у використанні. Ми оновлюємо структуру, дизайн та робимо сайт більш зрозумілим для користувачів',
        
        // Process
        'process': '[ Процеси ]',
        'process-title': 'Прозорий процес без хаосу. Ви знаєте, що відбувається на кожному етапі. Завжди є конкретний дедлайн',
        'process-title-mobile': 'Прозорий процес без хаосу. Ви знаєте, що відбувається на кожному етапі.',
        'process-title-p': '[ Завжди є конкретний дедлайн ]',
        'strategy': 'Стратегія',
        'strategy-desc': 'Аналізуємо бізнес, цільову аудиторію та конкурентів.<br /> Формуємо основу майбутнього сайту',
        'structure': 'Структура',
        'structure-desc': 'Формуємо логіку сторінок, сценарій користувача<br /> та створюємо прототип майбутнього сайту',
        'design': 'Дизайн',
        'design-desc': 'Розробляємо візуальну концепцію сайту,<br /> підбираємо стиль та створюємо дизайн сторінок',
        'development': 'Розробка',
        'development-desc': 'Верстаємо сайт, підключаємо необхідні<br /> функції та готуємо його до запуску',
        
        // Projects
        'projects-title': 'Сайти, створені для компаній<br />з різних сфер бізнесу',
        'projects-label': '[ Проєкти ]',
        'project-01': '/ Маркетингова агенція',
        'project-02': '/ Криптошкола',
        'project-03': '/ Спортивний клуб',
        'project-04': '/ Дизайн інтерʼєру ',
        'project-05': '/ Житловий комплекс',
        'view-projects': 'Переглянути проекти',
        
        // Contacts
        'contacts': '[ Контакти ]',
        'contacts-title': 'Обговоримо ваш проєкт',
        'messengers': 'Напишіть у зручний месенджер:',
        
        // Footer
        'footer-about': 'Про нас',
        'footer-processes': 'Процеси',
        'footer-projects': 'Проєкти',
        'footer-contacts': 'Контакти',
        
        // Burger menu
        'menu': '[ Меню ]',
        'burger-about': 'Про нас',
        'burger-processes': 'Процеси',
        'burger-projects': 'Проєкти',
        'burger-contacts': 'Контакти',
        
        // Works page

        'cursor-view': 'Переглянути →',
        'work-tag': 'Проєкти',
        'project-01': '/ Маркетингова агенція',
        'project-02': '/ Криптошкола',
        'project-03': '/ Спортивний клуб',
        'project-04': '/ Дизайн інтерʼєру ',
        'project-05': '/ Житловий комплекс',
        
        
        // Project pages
        'task': 'Задача',
        'solution': 'Рішення',
        'result': 'Результат',
        'project-01-desc': 'Лендінг для презентації бренду,<br /> послуг та команди',
        'project-01-task': 'Створити сайт, який чітко презентує агенцію,<br />пояснює її підхід та формує довіру у<br />потенційних клієнтів',
        'project-01-solution': 'Обрано чорно-білу стилістику з акцентом на контраст та типографіку. Візуальні елементи з людьми підсилюють ідею бренду та асоціацію з назвою агенції',
        'project-01-result': 'Структурований лендінг із блоками: послуги, команда, відгуки та форма заявки, який зручно сприймається та веде користувача до контакту',
        
        // Project 02
        'project-02': '/ Крипто-школа',
        'project-02-desc': 'Лендінг для продажу навчальних<br />курсів з трейдингу',
        'project-02-task': 'Створити сайт, який просто та зрозуміло<br />пояснює цінність курсу, структурує<br />інформацію та веде користувача до покупки',
        'project-02-solution': 'Візуальну основу побудовано навколо<br />фірмової 3D-фігури, яка формує впізнаваність<br />бренду та використовується як навігаційний<br />елемент. Оранжевий колір використано як<br />акцент для ключових дій',
        'project-02-result': 'Структурований лендінг, який послідовно<br />веде користувача від знайомства з<br />продуктом до оформлення заявки',
        
        // Project 03
        'project-03': '/ Падел-клуб',
        'project-03-alt': 'Спортивний клуб',
        'project-03-desc': 'Багатосторінковий сайт для презентації<br />клубу та запису на тренування',
        'project-03-task': 'Передати атмосферу клубу, підкреслити<br />його характер та зробити зручний запис<br />на тренування',
        'project-03-solution': 'Обрано вінтажну стилістику, що відображає<br />характер клубу та створює відчуття історії<br />й спільноти. Структура сайту включає:<br />про клуб, команду, історію, запис на<br />тренування та розділ з фірмовим мерчем',
        'project-03-result': 'Зрозумілий багатосторінковий сайт, який<br />поєднує презентацію бренду клубу та<br />зручну взаємодію з користувачем',
        
        // Project 04
        'project-04-4': '/ Студія дизайну інтер\'єру',
        'project-04-desc': 'Редизайн багатосторінкового сайту для<br />компанії, що спеціалізується на медичних просторах',
        'project-04-task': 'Оновити візуальний стиль сайту та зробити<br />його більш сучасним, зберігаючи існуючу<br />структуру та побажання клієнта по дизайну',
        'project-04-solution': 'Проєкт реалізовано на основі готової<br /> структури та референсів від клієнта.<br />Дизайн виконано у стриманій бежево-<br />коричневій палітрі, що підкреслює<br /> професійність та специфіку медичної сфери. Сайт включає сторінки: про<br />компанію, послуги та форму заявки<br /> для зручної взаємодії',
        'project-04-result': 'Оновлений сайт із сучасним дизайном,<br />який зберігає логіку структури та краще<br />презентує компанію онлайн',
        
        // Project 05
        'project-05': '/ Житловий комплекс',
        'project-05-desc': 'Сайт для презентації житлового комплексу',
        'project-05-task': 'Створити сайт, який презентує переваги житлового комплексу',
        'project-05-solution': 'Преміальна візуальна стилістика з акцентом на комфорт',
        'project-05-result': 'Інформативний сайт з каталогом квартир та формою запиту',
        
        // Language button
        'lang-en': 'EN',
        'lang-uk': 'UA'
    },
    en: {
        
       
        
        // Header
        'design-studio': 'Design studio',
        
        // Hero
        'hero-text': 'Landing pages and corporate websites<br />for companies that want to look<br />modern and attract new customers',
        'hero-text-mobile': 'Landing pages and corporate websites for companies that want to look modern and attract new customers',
        'hero-title': 'Structure.<br /> Design. Result.',
        'hero-title-mobile': 'Structure. Design. Result.',
        'discuss-project': 'Discuss project',
        
        // About
        'about-us': '[ About us ]',
        'about-title': 'We help companies<br />look professional online',
        'about-title-mobile': 'We help companies look professional online',
        'about-desc': 'We combine strategy, design and development<br />to create websites that work for business',
        'about-desc-mobile': 'We combine strategy, design and development to create websites that work for business',
        
        // Services
        'service-01-title': 'Landing Page',
        'service-01-desc': 'Suitable for launching a new product, running ad campaigns, or presenting a specific service. Helps clearly communicate your offer, highlight key benefits, and guide users toward conversion',
        'service-02-title': 'Corporate website',
        'service-02-desc': 'Suitable for companies that need a complete online presence. Allows you to present your business, services, and projects, and build a strong professional image',
        'service-03-title': 'Website redesign',
        'service-03-desc': 'Suitable if your current website looks outdated or is difficult to use. We improve structure, update the design, and make the website clearer and more user-friendly',
        
        // Process
        'process': '[ Process ]',
        'process-title': 'A clear process without chaos. You always<br/ > know what\'s happening at every stage.There is<br/ > always a specific deadline ',
        'process-title-mobile': 'A clear process without<br/ > chaos. You always know<br/ > what\'s happening at every stage.',
        'process-title-p': '[ There is always a specific deadline ]',
        'strategy': 'Strategy',
        'strategy-desc': 'We analyze your business, target audience, and competitors. We define the foundation of the future website',
        'structure': 'Structure',
        'structure-desc': 'We build the page structure and user flow, and create a prototype of the website',
        'design': 'Design',
        'design-desc': 'We develop the visual concept, define the style, and design the pages',
        'development': 'Development',
        'development-desc': 'We build the website, implement required features, and prepare it for launch',
        
        // Projects
        'projects-title': 'Websites created for companies<br />across different industries',
        'projects-label': '[ Projects ]',
        'project-01': '/ Marketing agency',
        'project-02': '/ Crypto school',
        'project-03': '/ Sports Club',
        'project-04': '/ Interior Design Studio',
        'project-05': '/ Residential complex',
        'view-projects': 'View projects',
        
        // Contacts
        'contacts': '[ Contacts ]',
        'contacts-title': 'Let\'s discuss your project',
        'messengers': 'Write to a convenient messenger:',
        
        // Footer
        'footer-about': 'About',
        'footer-processes': 'Process',
        'footer-projects': 'Projects',
        'footer-contacts': 'Contacts',
        
        // Burger menu
        'menu': '[ Menu ]',
        'burger-about': 'About',
        'burger-processes': 'Process',
        'burger-projects': 'Projects',
        'burger-contacts': 'Contacts',
        
        // Works page
        'cursor-view': 'View →',
        'work-tag': 'Projects',
        'project-01': '/ Marketing agency',
        'project-02': '/ Crypto school',
        'project-03': '/ Sports Club',
        'project-04': '/ Interior Design Studio',
        'project-05': '/ Residential complex',
        
        // Project pages
        'task': 'Task',
        'solution': 'Solution',
        'result': 'Result',
        'project-01-desc': 'Landing page for presenting the<br />brand, services, and team',
        'project-01-task': 'Create a website that clearly presents the agency, explains its approach and builds trust with potential clients',
        'project-01-solution': 'A black-and-white visual style was chosen with a focus on contrast and typography. Human-centered visuals reinforce the brand idea and reflect the meaning behind the agency\'s name',
        'project-01-result': 'A structured landing page with key sections: services, team, testimonials, and a contact form – designed to be easy to navigate and guide users toward inquiry',
        
        // Project 02
        'project-02': '/ Crypto school',
        'project-02-desc': 'Landing page for selling trading courses',
        'project-02-task': 'Create a website that clearly explains the value of the course, structures the information, and guides the user toward purchase',
        'project-02-solution': 'The visual concept is built around a branded 3D character that enhances recognition and is used as a navigation element. Orange is used as an accent color for key actions – buttons, pricing, and important sections –helping to focus user attention. The structure includes: about the company, course overview, program, testimonials, pricing options, FAQ, and a contact form',
        'project-02-result': 'A structured landing page that consistently guides the user from introduction to the product through to submitting an application',
        
        // Project 03
        'project-03': '/ Sports Club',
        'project-03-alt': 'Sports club',
        'project-03-desc': 'Multi-page website for presenting the club and booking training sessions',
        'project-03-task': 'Convey the atmosphere of the club, highlight its character, and make the booking process simple and convenient',
        'project-03-solution': 'A vintage-inspired visual style was chosen to reflect the club\'s character and create a sense of history and community. The structure includes: about the club, team, history, training booking, and a merchandise section',
        'project-03-result': 'A clear multi-page website that combines brand presentation with a seamless user experience',
        
        // Project 04
        'project-04': '/ Interior Design Studio',
        'project-04-desc': 'Redesign of multi-page website for a company specializing in medical spaces',
        'project-04-task': 'Redesign of a multi-page website for a company specializing in medical spaces',
        'project-04-solution': 'The project was based on an existing structure and client-provided references. The design is completed in a restrained beige and brown palette, reflecting professionalism and the specifics of the medical field. The website includes pages: about the company, services, and a contact form for convenient interaction',
        'project-04-result': 'An updated website with a modern design that preserves the original structure and presents the company more effectively online',
        
        // Project 05
        'project-05': '/ Residential complex',
        'project-05-desc': 'Website for residential complex presentation',
        'project-05-task': 'Create a website that presents the advantages of the residential complex',
        'project-05-solution': 'Premium visual style with emphasis on comfort',
        'project-05-result': 'Informative website with apartment catalog and inquiry form',
        

        'project-01-p':'Marketing Agency',
        'project-02-p':'Crypto School',
        'project-03-p':'Sports Club',
        'project-04-p':'Interior Design Studio',
        'project-05-p':'Residential complex',
        







        // Language button
        'lang-en': 'EN',
        'lang-uk': 'UA'
    }
};

// Current language (default: Ukrainian)
let currentLang = localStorage.getItem('lang') || 'uk';

// Apply translations to the page
function applyTranslations() {
    const elements = document.querySelectorAll('[data-i18n]');
    const isMobile = window.innerWidth <= 768;
    
    elements.forEach(el => {
        let key = el.getAttribute('data-i18n');
        
        // Проверяем мобильную версию
        if (isMobile && el.classList.contains('mobile')) {
            const mobileKey = key + '-mobile';
            if (translations[currentLang] && translations[currentLang][mobileKey]) {
                key = mobileKey;
            }
        }
        // Проверяем версию для рабочего стола
        else if (!isMobile && el.classList.contains('desktop')) {
            const desktopKey = key + '-desktop';
            if (translations[currentLang] && translations[currentLang][desktopKey]) {
                key = desktopKey;
            }
        }
        
        if (translations[currentLang] && translations[currentLang][key]) {
            // Используем innerHTML для сохранения тегов <br />
            el.innerHTML = translations[currentLang][key];
        }
    });
    
    // Обновляем текст кнопки языка
    const langBtn = document.querySelector('.language span');
    if (langBtn) {
        langBtn.textContent = currentLang === 'uk' ? 'UA' : 'EN';
    }
    
    // Обновляем класс кнопки для стилизации
    const langBtnContainer = document.querySelector('.language');
    if (langBtnContainer) {
        langBtnContainer.setAttribute('data-lang', currentLang);
    }
    
    // Сохраняем выбор языка
    localStorage.setItem('lang', currentLang);
}

// Переключение языка
function toggleLanguage() {
    currentLang = currentLang === 'uk' ? 'en' : 'uk';
    applyTranslations();
}

// Инициализация языка при загрузке страницы
document.addEventListener('DOMContentLoaded', () => {
    applyTranslations();
    
    // Добавляем обработчик клика на кнопку языка
    const langBtn = document.querySelector('.language');
    if (langBtn) {
        langBtn.addEventListener('click', toggleLanguage);
    }
});

// ================================================
// LOADER
// ================================================
const loaderPercent = document.getElementById('loaderPercent');
const loaderBar = document.getElementById('loaderBar');
const loader = document.getElementById('loader');

if (loader) {
    let progress = 0;
    const interval = setInterval(() => {
        progress += Math.random() * 3 + 1;
        if (progress >= 100) {
            progress = 100;
            clearInterval(interval);
            setTimeout(() => {
                loader.style.opacity = '0';
                setTimeout(() => {
                    loader.style.display = 'none';
                    const first2 = document.querySelector('.first2');
                    if (first2) first2.classList.add('visible');

                    // Показываем баннер после загрузчика
                    const banner = document.getElementById('cookie-banner');
                    if (banner && !document.cookie.split('; ').some(r => r.startsWith('cookie_consent='))) {
                        banner.classList.add('show');
                    }
                }, 600);
            }, 300);
        }
        if (loaderPercent) loaderPercent.textContent = Math.floor(progress) + '%';
        if (loaderBar) loaderBar.style.width = progress + '%';
    }, 40);
}


// ================================================
// COOKIE BANNER
// ================================================
document.addEventListener('DOMContentLoaded', () => {
    const banner = document.getElementById('cookie-banner');
    const acceptBtn = document.getElementById('accept-btn');
    const rejectBtn = document.getElementById('reject-btn');

    if (!banner) return;

    function hasConsent() {
        return localStorage.getItem('cookie_consent') !== null;
    }

    // Показываем только если еще не было выбора
    // и только после того как загрузчик закрылся
    if (!hasConsent()) {
        // Ждем пока загрузчик закроется
        const waitForLoader = setInterval(() => {
            const loader = document.getElementById('loader');
            if (!loader || loader.style.display === 'none') {
                clearInterval(waitForLoader);
                setTimeout(() => {
                    banner.classList.add('show');
                }, 500);
            }
        }, 100);
    }

    if (acceptBtn) {
        acceptBtn.addEventListener('click', () => {
            localStorage.setItem('cookie_consent', 'accepted');
            banner.classList.remove('show');
        });
    }

    if (rejectBtn) {
        rejectBtn.addEventListener('click', () => {
            localStorage.setItem('cookie_consent', 'rejected');
            banner.classList.remove('show');
        });
    }
});

// ================================================
// CURSOR (works page)
// ================================================
const cursorBtn = document.getElementById('cursorBtn');
const imgWraps = document.querySelectorAll('.work-img-wrap, .work-img-wrap2');

if (cursorBtn) {
    let currentHref = '#';

    document.addEventListener('mousemove', (e) => {
        cursorBtn.style.left = e.clientX + 'px';
        cursorBtn.style.top  = e.clientY + 'px';
    });

    imgWraps.forEach(wrap => {
        const link = wrap.getAttribute('data-href');

        wrap.addEventListener('mouseenter', () => {
            currentHref = link || '#';
            cursorBtn.classList.add('visible');
        });

        wrap.addEventListener('mouseleave', () => {
            cursorBtn.classList.remove('visible');
        });

        wrap.addEventListener('mousedown', () => {
            cursorBtn.classList.add('clicking');
        });

        wrap.addEventListener('mouseup', () => {
            cursorBtn.classList.remove('clicking');
            if (currentHref && currentHref !== '#') {
                window.location.href = currentHref;
            }
        });
    });
}

// Клик работает всегда — и без курсора и на мобиле
imgWraps.forEach(wrap => {
    wrap.addEventListener('click', () => {
        const link = wrap.getAttribute('data-href');
        if (link) window.location.href = link;
    });
});

// ================================================
// BURGER MENU
// ================================================
const burger = document.getElementById('burger');
const burgerMenu = document.getElementById('burgerMenu');
const burgerClose = document.getElementById('burgerClose');

if (burger && burgerMenu) {
    burger.addEventListener('click', () => {
        burgerMenu.classList.add('open');
    });
}

if (burgerClose && burgerMenu) {
    burgerClose.addEventListener('click', () => {
        burgerMenu.classList.remove('open');
    });
}

// Закрытие при клике на ссылку меню
if (burgerMenu) {
    burgerMenu.querySelectorAll('.bm-link').forEach(link => {
        link.addEventListener('click', () => {
            burgerMenu.classList.remove('open');
        });
    });
}

// ================================================
// ACCORDION
// ================================================
function toggle(header) {
    if (!header || !header.nextElementSibling) return;
    const body = header.nextElementSibling;
    const isOpen = body.classList.contains('open');

    document.querySelectorAll('.service-body').forEach(b => b.classList.remove('open'));
    document.querySelectorAll('.service-header').forEach(h => h.classList.remove('active'));

    if (!isOpen) {
        body.classList.add('open');
        header.classList.add('active');
    }
}

// ================================================
// BACK TO TOP
// ================================================
const backToTopBtn = document.getElementById('backToTop');
if (backToTopBtn) {
    backToTopBtn.addEventListener('click', () => {
        window.scrollTo({
            top: 0,
            behavior: 'smooth'
        });
    });
}